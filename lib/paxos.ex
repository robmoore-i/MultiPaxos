...
def main do
  ...
  IO.puts "nClients,nServers,printAfter,maxRequests,backoffInitial,backoffMultiplier,backoffReducer"
  IO.puts ["#{config.n_clients},#{config.n_servers},#{config.print_after},#{config.max_requests},#{config.backoff_initial},#{config.backoff_multiplier},#{config.backoff_reducer}"]
